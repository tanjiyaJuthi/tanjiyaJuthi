...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus("error");
      setMessage("Connection error. Please check your internet or try again later.");
    }
  };

  return (
    <section className="py-16 md:py-24 max-w-container-max mx-auto px-4 md:px-gutter mb-10 md:mb-xl" id="contact">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-label-md text-on-surface-variant uppercase tracking-wider">Name</label>
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-label-md text-on-surface-variant uppercase tracking-wider">Email</label>
                    <input type="email" name="email" required placeholder="john@example.com" 
                      className="w-full bg-surface-container-highest/50 border border-outline-variant/30 rounded-xl px-4 py-3 text-on-surface focus:border-primary outline-none transition-all" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-label-md text-on-surface-variant uppercase tracking-wider">Subject</label>
                  <input type="text" name="subject_line" required placeholder="Project Inquiry" 
                    className="w-full bg-surface-container-highest/50 border border-outline-variant/30 rounded-xl px-4 py-3 text-on-surface focus:border-primary outline-none transition-all" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-label-md text-on-surface-variant uppercase tracking-wider">Message</label>
                  <textarea name="message" required rows={4} placeholder="Tell me about your project..." 
                    className="w-full bg-surface-container-highest/50 border border-outline-variant/30 rounded-xl px-4 py-3 text-on-surface focus:border-primary outline-none transition-all resize-none"></textarea>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {message && (
                    <p className={`text-center font-label-md text-sm ${status === "success" ? "text-primary" : "text-error"}`}>
                      {message}
                    </p>
                  )}
                </div>
